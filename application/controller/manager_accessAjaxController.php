<?php
 class manager_accessAjaxController extends basisAjaxController{
		...
		if(is_array($this->userArray) ){
			foreach($this->userArray as $value){
				...
				if($value->idStaff){
					...
					if( is_object($this->teacher)){ 	$arr = $this->teacher->getArrForXMLDocument(); }				
				}
				$arr['idUser'] = $value->id;				
	...
	public function deleteElementAction(){
		...
		if(is_object($this->user) ){ $this->accessRepository->deleteElement($this->user); }
	...
	public function updateTeacherForUserAction(){
		...
		$this->user = $this->accessRepository->getElementById($idUser);
		if(!is_object($this->teacher) || ($this->teacher->idAuthent)  || ($this->teacher->statusAuthent)  || ($this->teacher->nameAuthent) )	{ $this->getAllElementAction(); }
		if(!is_object($this->user) || ($this->user->status)  || ($this->user->idStaff) ){ $this->getAllElementAction(); }
