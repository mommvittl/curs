...
//			var_dump($this->auditorias);
	...
	public function elementsUpdateAction(){
		...
		if( !is_array( $arrInputValue ) ){ $this->returnXmlResponse( "<flagResult>1</flagResult><resultAddElement>Error...ошибка передачи данных</resultAddElement>" ) ; }
		$arrInputValue['work'] = '1';
		$this->auditorias = auditoriasModel::fromState( $arrInputValue );
		if( !is_object( $this->auditorias ) || !auditoriasValidate::validate( $this->auditorias ) ){ $this->returnXmlResponse( "<flagResult>2</flagResult><resultAddElement>Некорректные входные данные</resultAddElement>" ) ; }
		if( $paramAddUpd == 'update' ){
			$auditor = $this->auditoriasRepository->getElementById($idElement);
			if( !is_object( $auditor ) ){ $this->returnXmlResponse( "<flagResult>2</flagResult><resultAddElement>Некорректные входные данные</resultAddElement>" ) ; }
			...
			if( is_object( $auditor[0]) ){  $this->returnXmlResponse( "<flagResult>1</flagResult><resultAddElement>Такая аудитория уже есть в базе</resultAddElement>" ); }
